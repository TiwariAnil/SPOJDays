...
//     14 od
*/
int height[N]; // LCP(i, i-1);
//  height[i] = length of the longest common prefix of suffix pos[i] and suffix pos[i-1]
//  height[0] = 0


// Construction LCP [longest common prefix] array
void getHeight(int n)
{
    for (int i=0; i<n; ++i)          //Height[i]=lcp ( s[a[i-1], n] , s[a[i],n] )
		rank[pos[i]] = i;
    height[0] = 0;
    for (int i=0, h=0; i<n; ++i)
    {
        if (rank[i] > 0) // not the the 0th rank suffix
        {
            int j = pos[rank[i]-1];
            while (i+h<n && j+h<n && str[i+h]==str[j+h])
				h++;
            height[rank[i]] = h; //suffix with rank rank[i] is compressed at h height
            if (h > 0) h--;
        }
    }
}
...
*/
    ...
    }//  (n-pos[i]) is length of suffix and height[i] is the commn prefix with
    // pos[i-1]th suffix, each suffix contributes only the uncommon
    // part of it as a new substring.
    cout << ans << endl;
}
